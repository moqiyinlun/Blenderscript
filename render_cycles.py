...
def render_sequence_with_all_cameras(output_path,camera_path):
    setup_transparent_render()
    
    bpy.context.scene.frame_set(0)
    for obj in camera_path:
        bpy.context.scene.camera = obj
        bpy.context.scene.render.filepath = f"{output_path}/{obj.name}.png"
        print(f"{output_path}/{obj.name}.png")
...
render_sequence_with_all_cameras(output_directory,camera_list)